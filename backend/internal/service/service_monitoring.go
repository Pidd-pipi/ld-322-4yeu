...
func NewMonitoringService(g *repository.GreenhouseRepository, s *repository.SensorRepository, a *repository.AlertRepository, rules *RuleService, l *slog.Logger, h *ws.Hub) *MonitoringService {
	return &MonitoringService{g, s, a, rules, l, h}
}
...
func (s *MonitoringService) Ingest(sensorID uint, value float64) (*model.SensorReading, *model.Alert, error) {
	...
	if s.rules != nil {
		s.rules.EvaluateReading(sensor, value)
	}
