...
func New(d Dependencies) *gin.Engine {
	...
	automation := handler.NewAutomationHandler(d.Automation, v)
	...
	api.GET("/automation-rules", automation.List)
	...
	secured.POST("/automation-rules", automation.Create)
	secured.PATCH("/automation-rules/:id/status", automation.SetEnabled)
	secured.DELETE("/automation-rules/:id", automation.Delete)
