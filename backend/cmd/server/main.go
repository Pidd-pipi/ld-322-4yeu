...
func main() {
	...
	ruleService := service.NewRuleService(rulesRepo, sensors, devices, control, l, hub)
	monitoring := service.NewMonitoringService(greenhouses, sensors, alerts, ruleService, l, hub)
	alertService := service.NewAlertService(alerts, l)
	reports := service.NewReportService(sensors, alerts)
	engine := router.New(router.Dependencies{Config: cfg, Logger: l, Auth: auth, Monitoring: monitoring, Alerts: alertService, Control: control, Rules: ruleService, Reports: reports, Hub: hub})
