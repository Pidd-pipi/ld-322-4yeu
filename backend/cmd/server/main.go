...
func main() {
	...
	automationRules := repository.NewAutomationRuleRepository(db)
	hub := ws.NewHub()
	auth := service.NewAuthService(cfg.JWTSecret)
	automation := service.NewAutomationService(db, greenhouses, automationRules, sensors, devices, l, hub)
	monitoring := service.NewMonitoringService(greenhouses, sensors, alerts, automation, l, hub)
	...
	engine := router.New(router.Dependencies{Config: cfg, Logger: l, Auth: auth, Monitoring: monitoring, Alerts: alertService, Control: control, Automation: automation, Reports: reports, Hub: hub})
